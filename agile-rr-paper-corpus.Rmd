...
This document is versioned in a public [git](https://git-scm.com/) repository, [https://github.com/nuest/reproducible-research-and-giscience](https://github.com/nuest/reproducible-research-and-giscience), and the current revision is `r system2("git", "rev-parse --short HEAD", stdout = TRUE, stderr = TRUE)`.
...
Add the PDFs to a directory called ` `r data_path` ` next to this file.

You can contact the original paper authors and ask for the test dataset to reproduce the full analysis.
Alternatively, you can download a selection of AGILE short papers to test the workflow using the code below.
...
The **plots and tables** of survey data and evaluation use the packages [`ggplot2`](http://ggplot2.tidyverse.org/) and [`knitr::kable()`](https://yihui.name/knitr/)/[`kableExtra`](https://cran.r-project.org/package=kableExtra).
